...
use bytes::Bytes;
use cntryl_midge::sst::block_meta::{BlockMeta, BlockIndexEntry};
...
fn should_check_range_intersection() {
    ...
    // Assert: Non-intersecting ranges
...
fn should_track_multiple_tombstone_ranges() {
    ...
    let block1 = BlockMeta::new(Bytes::from("a"), Bytes::from("c"), BlockHandle::new(0, 100))
        .with_tombstones(true, Some(Bytes::from("a")), Some(Bytes::from("c")));
    ...
    // Assert
    assert!(block1.might_be_fully_covered());
    assert!(!block2.might_be_fully_covered()); // Not fully covered (starts before tombstone)
}
...
fn should_maintain_fence_pointer_ordering() {
    ...
    assert!(block1.max_key <= block2.min_key);
    assert!(block2.max_key <= block3.min_key);
}
