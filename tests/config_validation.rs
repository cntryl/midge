...
//!
//! ## Coverage
//! - Invalid configuration rejection (memtable size, bounds)
//! - Concurrent configuration validation
//! - Config stability under stress
//! - Config persistence across restarts
//!
//! ## Storage Mode Coverage
//! Most tests use LocalDisk mode since config validation is storage-agnostic.
...
#[test]
fn should_reject_config_given_memtable_size_exceeds_reasonable_bounds_when_opening() {
    // Arrange
...
#[test]
fn should_accept_config_given_valid_memtable_size_when_opening() {
    // Arrange
    let dir = test_temp_dir();
    ...
    let opts = MidgeOptions {
        ...
        memtable_size: 64 * 1024, // 64KB - reasonable size
        ..Default::default()
    };
    ...
    // Assert
    assert!(result.is_ok(), "Should accept valid memtable_size config");
}
...
#[test]
fn should_validate_configs_independently_given_concurrent_engine_opens_when_parallel() {
    // Arrange
...
#[test]
fn should_maintain_stability_given_rapid_writes_when_valid_config() {
    // Arrange
...
#[test]
fn should_recover_data_given_same_config_when_engine_reopened() {
    // Arrange
    let dir = test_temp_dir();
...
#[test]
fn should_remain_functional_given_config_unchanged_when_operations_continue() {
    // Arrange
    let (_dir, eng) = new_engine();
    let cf = eng.default_column_family();
    eng.put(&cf, b"key1", b"value1").expect("put");

    // Act - continue operations (config unchanged)
    eng.put(&cf, b"key2", b"value2").expect("put after");
    eng.flush().ok();
