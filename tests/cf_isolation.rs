...
fn should_isolate_keys_across_column_families() {
    ...
    engine.put_cf(&cf1, b"key", b"value_cf1").expect("put cf1");
    engine.put_cf(&cf2, b"key", b"value_cf2").expect("put cf2");

    // Assert
    let result1 = engine.get_cf(&cf1, b"key").expect("get cf1");
    let result2 = engine.get_cf(&cf2, b"key").expect("get cf2");
...
fn should_not_see_deleted_keys_from_other_cfs() {
    ...
    engine.put_cf(&cf1, b"key", b"value1").expect("put cf1");
    engine.put_cf(&cf2, b"key", b"value2").expect("put cf2");
    
    // Act - Delete from cf1 only
    engine.delete_cf(&cf1, b"key").expect("delete cf1");
...
fn should_scan_only_within_column_family() {
    ...
    engine.put_cf(&cf1, b"key1", b"cf1_value1").expect("put");
    engine.put_cf(&cf1, b"key2", b"cf1_value2").expect("put");
    engine.put_cf(&cf1, b"key3", b"cf1_value3").expect("put");
    
    engine.put_cf(&cf2, b"key1", b"cf2_value1").expect("put");
    engine.put_cf(&cf2, b"key2", b"cf2_value2").expect("put");
    engine.put_cf(&cf2, b"key3", b"cf2_value3").expect("put");
...
fn should_isolate_after_flush() {
    ...
    engine.put_cf(&cf1, b"key2", b"value2_cf1").expect("put cf1");
    engine.put_cf(&cf2, b"key2", b"value2_cf2").expect("put cf2");

    // Act & Assert - Read from both CFs
    let result1 = engine.get_cf(&cf1, b"key").expect("get cf1");
    let result2 = engine.get_cf(&cf2, b"key").expect("get cf2");
    let result3 = engine.get_cf(&cf1, b"key2").expect("get cf1 key2");
    let result4 = engine.get_cf(&cf2, b"key2").expect("get cf2 key2");
...
fn should_isolate_across_restart() {
    ...
    // First session: write data to both CFs
    {
        ...
        engine.put_cf(&cf1, b"shared_key", b"cf1_value").expect("put cf1");
        engine.put_cf(&cf2, b"shared_key", b"cf2_value").expect("put cf2");
    }
    ...
    // Act
    let result1 = engine2.get_cf(&cf1, b"shared_key").expect("get cf1");
    let result2 = engine2.get_cf(&cf2, b"shared_key").expect("get cf2");
...
fn should_not_read_default_cf_data_from_custom_cf() {
    ...
    // Write to default CF
    engine.put_cf(&default_cf, b"key", b"default_value").expect("put default");

    // Act - Try to read from custom CF
    let result = engine.get_cf(&custom_cf, b"key").expect("get custom");

    // Assert
    assert_eq!(result, None, "Custom CF should not see default CF data");
    
    // Verify data exists in default CF
    let default_result = engine.get_cf(&default_cf, b"key").expect("get default");
...
fn should_handle_concurrent_writes_to_different_cfs() {
    ...
    for i in 0..100 {
        ...
        engine.put_cf(&cf1, key.as_bytes(), b"cf1_val").expect("put cf1");
        engine.put_cf(&cf2, key.as_bytes(), b"cf2_val").expect("put cf2");
    }

    // Assert - Verify all writes went to correct CFs
    for i in 0..100 {
        let key = format!("key_{:03}", i);
        let val1 = engine.get_cf(&cf1, key.as_bytes()).expect("get cf1");
        let val2 = engine.get_cf(&cf2, key.as_bytes()).expect("get cf2");
