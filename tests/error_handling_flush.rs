...
fn should_preserve_memtable_data_when_crash_during_flush_before_manifest_update() {
    ...
    let opts = MidgeOptions { test_hooks: Some(hooks.clone()), ..opts };
    ...
    let opts_reopen = MidgeOptions { test_hooks: None, ..opts };
    ...
    assert!(first.is_some() && mid.is_some() && last.is_some(), "All written keys should be present after recovery from WAL");
}
...
fn should_resume_flush_when_flush_gate_released() {
    ...
    eng.wait_for_flush(Duration::from_secs(3)).expect("flush completion");

    // Assert - data should be persisted (keys still readable)
    let sample = eng.get(&cf, b"key0000").expect("get sample");
    assert!(sample.is_some(), "Sample key should remain readable after flush");
}
...
fn should_not_leave_partial_sst_files_when_crash_during_flush_manifest_update() {
    ...
    let cf = eng.default_column_family();
    for i in [0, 35, 69] {
        let res = eng.get(&cf, format!("key{:04}", i).as_bytes()).expect("get");
        assert!(res.is_some(), "Key {:04} should be present after recovery", i);
    }
}
...
fn should_recover_fsynced_data_when_crash_during_flush_before_manifest_update() {
    ...
    for idx in [0, 30, 60, 89] {
        let res = eng.get(&cf, format!("key{:04}", idx).as_bytes()).expect("get");
        assert!(res.is_some(), "Key {:04} should be present after recovery", idx);
    }
}
