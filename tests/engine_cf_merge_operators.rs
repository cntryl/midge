...
fn should_resolve_merge_using_cf_specific_operator_when_different_operators_registered() {
    ...
    engine
        .register_merge_operator(&cf_numbers, Arc::new(IntegerAddOperator));

    // Register StringAppendOperator for strings CF
    engine
        .register_merge_operator(&cf_strings, Arc::new(StringAppendOperator::new(b",")));

    // Act - Write merges to both CFs
    engine.put(&cf_numbers, b"counter", b"10").expect("put number");
    engine.merge_cf(&cf_numbers, b"counter", b"5").expect("merge number 1");
    engine.merge_cf(&cf_numbers, b"counter", b"3").expect("merge number 2");

    engine.put(&cf_strings, b"log", b"first").expect("put string");
    engine.merge_cf(&cf_strings, b"log", b"second").expect("merge string 1");
    engine.merge_cf(&cf_strings, b"log", b"third").expect("merge string 2");
    ...
    assert_eq!(string_result, Some(Bytes::from_static(b"first,second,third")));
}
...
fn should_resolve_merge_correctly_after_flush_when_per_cf_operator_registered() {
    ...
    engine
        .register_merge_operator(&cf, Arc::new(IntegerAddOperator));
...
fn should_handle_merge_after_delete_when_per_cf_operator_registered() {
    ...
    engine
        .register_merge_operator(&cf, Arc::new(StringAppendOperator::new(b"-")))
        ;
...
fn should_isolate_merge_operators_across_cfs_when_concurrent_flushes() {
    ...
    engine
        .register_merge_operator(&cf_alpha, Arc::new(IntegerAddOperator))
        ;
    engine
        .register_merge_operator(&cf_beta, Arc::new(StringAppendOperator::new(b"|")))
        ;
    ...
    assert_eq!(
        beta_result,
        Some(Bytes::from_static(b"item0|item1|item2|item3|item4|item5|item6|item7|item8|item9"))
    );
}
...
fn should_handle_default_cf_merge_independently_from_other_cfs() {
    ...
    engine
        .register_merge_operator(&default_cf, Arc::new(IntegerAddOperator))
        ;
    engine
        .register_merge_operator(&custom_cf, Arc::new(StringAppendOperator::new(b":")))
        ;

    // Act - Write merges to both CFs
    engine.put(&default_cf, b"count", b"0").expect("put default");
    engine.merge_cf(&default_cf, b"count", b"1").expect("merge default 1");
    engine.merge_cf(&default_cf, b"count", b"2").expect("merge default 2");

    engine.put(&custom_cf, b"path", b"root").expect("put custom");
    engine.merge_cf(&custom_cf, b"path", b"dir").expect("merge custom 1");
    engine.merge_cf(&custom_cf, b"path", b"file").expect("merge custom 2");
