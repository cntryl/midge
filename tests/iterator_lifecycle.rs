...
fn should_continue_iteration_given_compaction_in_progress_when_scan() {
    ...
    assert_eq!(results.len(), 200, "All keys should be visible during compaction");
}
...
fn should_rewind_iterator_to_start_given_reset_called() {
    ...
    assert_eq!(results1, results2, "Repeated scans should produce identical results");
}
...
fn should_resume_iteration_given_checkpoint_sequence() {
    ...
    let cf = eng.default_column_family();
    for i in 0..20 {
        eng.put(&cf, format!("key{:02}", i).as_bytes(), format!("value{}", i).as_bytes())
            .expect("put");
    }
...
fn should_iterate_in_reverse_given_reverse_iterator_enabled_when_scan() {
    ...
    assert_eq!(results[0].0, Bytes::from_static(b"key3"), "First result should be key3");
    assert_eq!(results[1].0, Bytes::from_static(b"key2"), "Second result should be key2");
    assert_eq!(results[2].0, Bytes::from_static(b"key1"), "Third result should be key1");
}
