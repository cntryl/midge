...
fn should_delete_keys_in_range_given_delete_range_when_querying() {
    for mode in disk_storage_modes() {
        ...
        assert_eq!(
            eng.get(&cf, b"b").expect("get"),
            None,
            "{}: b deleted",
            name
        );
        assert_eq!(
            eng.get(&cf, b"c").expect("get"),
            None,
            "{}: c deleted",
            name
        );
...
fn should_delete_keys_across_levels_given_flushed_data_when_delete_range() {
    for mode in disk_storage_modes() {
        ...
        eng.delete_range(&cf, b"key2", b"key5")
            .expect("delete_range");

        // Assert
        assert!(eng.get(&cf, b"key1").expect("get").is_some());
        assert!(
            eng.get(&cf, b"key2").expect("get").is_none(),
            "{}: key2",
            name
        );
        assert!(
            eng.get(&cf, b"key3").expect("get").is_none(),
            "{}: key3",
            name
        );
        assert!(
            eng.get(&cf, b"key4").expect("get").is_none(),
            "{}: key4",
            name
        );
...
fn should_handle_empty_range_given_start_equals_end_when_delete_range() {
    for mode in disk_storage_modes() {
        ...
        // Act - empty range
        eng.delete_range(&cf, b"key1", b"key1")
            .expect("delete_range");
...
fn should_handle_large_range_deletion_given_many_keys_when_deleting() {
    for mode in disk_storage_modes() {
        ...
        eng.delete_range(&cf, b"key000100", b"key000900")
            .expect("delete_range");

        // Assert - spot check
        assert!(
            eng.get(&cf, b"key000050").expect("get").is_some(),
            "{}",
            name
        );
        assert!(
            eng.get(&cf, b"key000500").expect("get").is_none(),
            "{}",
            name
        );
        assert!(
            eng.get(&cf, b"key000950").expect("get").is_some(),
            "{}",
            name
        );
        drop(eng);
...
fn should_persist_delete_range_given_wal_when_recovering() {
    for mode in disk_storage_modes() {
...

        {
            ...
            // Act - delete range
            eng.delete_range(&cf, b"key2", b"key4")
                .expect("delete_range");
        }
        ...
        assert_eq!(
            eng2.get(&cf2, b"key1").expect("get"),
        ...
        assert_eq!(
            eng2.get(&cf2, b"key4").expect("get"),
            Some(Bytes::from("value4")),
            "{}",
            name
        );
        assert_eq!(
            eng2.get(&cf2, b"key5").expect("get"),
...
fn should_recover_range_tombstone_given_no_flush_when_restarting() {
    for mode in disk_storage_modes() {
...

        {
            ...
            eng.delete_range(&cf, b"key1", b"key3")
                .expect("delete_range");
        }
...
fn should_apply_delete_range_after_crash_given_flushed_tombstone_when_recovering() {
    for mode in disk_storage_modes() {
        ...
        // Act
        {
            ...
            eng.delete_range(&cf, b"k020", b"k080")
                .expect("delete_range");
            eng.flush().expect("flush");
...
fn should_apply_range_tombstone_during_compaction_given_flushed_data_when_compacting() {
    for mode in disk_storage_modes() {
        ...
        eng.delete_range(&cf, b"key005", b"key015")
            .expect("delete_range");
        eng.flush().expect("flush");
...
fn should_not_resurrect_deleted_keys_given_compaction_when_range_delete_applied() {
    for mode in disk_storage_modes() {
        ...
        eng.delete_range(&cf, b"key0", b"key9")
            .expect("delete_range");
        eng.flush().expect("flush");
...
fn should_include_deleted_range_in_snapshot_scan_given_delete_after_snapshot_when_scanning() {
    for mode in disk_storage_modes() {
        ...
        for i in 0..100 {
            eng.put(&cf, format!("r{:03}", i).as_bytes(), b"v")
                .expect("put");
        }
        eng.flush().expect("flush");
        ...
        // Act - delete range after snapshot
        eng.delete_range(&cf, b"r020", b"r080")
            .expect("delete_range");
        eng.flush().expect("flush");
...
fn should_merge_overlapping_ranges_given_multiple_delete_ranges_when_deleting() {
    for mode in disk_storage_modes() {
        ...
        eng.delete_range(&cf, b"key02", b"key06")
            .expect("delete_range 1");
        eng.delete_range(&cf, b"key04", b"key08")
            .expect("delete_range 2");
