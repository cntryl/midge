...
fn should_detect_write_write_conflict_given_concurrent_updates_to_same_key() {
    ...
    let mut txn1 = engine.begin_transaction(&cf).expect("Transaction creation failed");
    let mut txn2 = engine.begin_transaction(&cf).expect("Transaction creation failed");
...
fn should_abort_second_transaction_given_write_conflict_when_both_commit() {
    ...
    let mut winner = engine.begin_transaction(&cf).expect("Transaction creation failed");
    let mut loser = engine.begin_transaction(&cf).expect("Transaction creation failed");

    winner
        .put(b"conflict_key", b"txn1_value")
        .unwrap();
    loser
        .put(b"conflict_key", b"txn2_value")
        .unwrap();
...
fn should_handle_write_conflict_on_delete_given_concurrent_delete_and_put() {
    ...
    let mut delete_txn = engine.begin_transaction(&cf).expect("Transaction creation failed");
    let mut put_txn = engine.begin_transaction(&cf).expect("Transaction creation failed");
...
fn should_detect_conflict_on_delete_range_given_overlapping_keys() {
    ...
    let mut range_txn = engine.begin_transaction(&cf).expect("Transaction creation failed");
    range_txn
        .delete_range(b"key3", b"key7")
        .unwrap();

    let mut overlap_txn = engine.begin_transaction(&cf).expect("Transaction creation failed");
    overlap_txn
        .put(b"key5", b"new_value")
        .unwrap();
