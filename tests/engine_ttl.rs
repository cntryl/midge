...
#[test]
fn should_maintain_snapshot_consistency_when_concurrent_ttl_and_compaction() {
    for_each_storage_mode(&all_storage_modes_new(), |mode, opts| {
        eprintln!(
            "\n=== TTL: Concurrent Expiry and Compaction (mode: {}) ===\",
            mode
        );
        ...
        let mut tx = engine
            ...
            .unwrap();
        for i in 0..100 {
            let key = format!("concurrent_ttl_key_{:04}", i);
            tx.put(key.as_bytes().to_vec(), b"concurrent_value".to_vec(), Some(2))
                .unwrap();
        }
        engine.commit(tx, WriteOptions::buffered()).unwrap();
        engine.flush_cf(&cf).expect("flush");

        // Create snapshot before expiry
        let snapshot = engine
            .begin_tx(cf.id(), TransactionMode::ReadOnly)
            .unwrap();

        // Wait for ~50% TTL elapsed
        thread::sleep(Duration::from_millis(1000));

        // Act: Trigger compaction while snapshot active and TTL expiring
        ...
        // Snapshot reads during expiry and compaction
        let snap_reads = (0..100)
            .filter(|i| {
                let key = format!("concurrent_ttl_key_{:04}", i);
                snapshot.get(key.as_bytes()).ok().flatten().is_some()
            })
            .count();

        compact_handle.join().ok();

        // Assert: Snapshot returns consistent results
        // Either all values (snapshot sees pre-expiry data)
        // Or all None (if snapshot taken after expiry passed)
        assert!(
            snap_reads == 100 || snap_reads == 0,
            "snapshot returned inconsistent TTL state: {}",
            snap_reads
        );

        // Release snapshot; verify post-compaction state
        drop(snapshot);
        ...
        assert_eq!(post_compact_reads, 0, "all TTL keys should expire");

        eprintln!("✓ Concurrent TTL expiry and compaction handled correctly");
    });
}
