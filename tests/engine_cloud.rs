    ...
    match put_result {
        Ok(()) => {
            engine
                ...
                .unwrap();
            eprintln!("âœ“ Put to cloud succeeded");
            ...
            match tx.get(b"cloud_key") {
                Ok(Some(val)) if val.as_ref() == b"cloud_value" => {
                    eprintln!("âœ“ Get from cloud succeeded");
                }
                Ok(Some(val)) => {
                    eprintln!(
                        "âœ— Unexpected value from cloud: {:?}",
                        String::from_utf8_lossy(val.as_ref()).to_string()
                    );
                }
                Ok(None) => {
                    eprintln!("âœ— Key not found in cloud (data loss?)");
                }
                Err(e) => {
        ...
        Err(e) => {
            eprintln!("âœ— Cloud put failed: {}", e);
    ...
    match put_result {
        Ok(()) => {
            ...
            match engine.commit(txn, cntryl_midge::WriteOptions::buffered()) {
                Ok(()) => {
                    eprintln!("âœ“ Cloud transaction committed");
                    ...
                    match tx.get(b"tx_key1") {
                        Ok(Some(_)) => {
                            eprintln!("âœ“ Transaction data persisted to cloud");
                        }
                        _ => {
                            eprintln!("âœ— Transaction data not found in cloud");
                        }
                    }
                }
                Err(e) => {
                    eprintln!("âœ— Cloud transaction commit failed: {}", e);
                }
            }
        }
        Err(e) => {
...
fn should_support_range_scans_on_cloud() {
    ...
    match scan {
        Ok(mut iterator) => {
            ...
            eprintln!("âœ“ Cloud scan succeeded: {} keys", count);

            if count >= 20 {
                eprintln!("âœ“ All keys present in cloud scan");
            } else {
                eprintln!("âœ— Incomplete scan: < 20 keys expected");
            }
        }
        Err(e) => {
...
fn should_support_snapshots_on_cloud_data() {
    ...
    match snapshot.get(b"snap_key") {
        Ok(Some(val)) => {
            eprintln!(
                "âœ“ Snapshot read from cloud: {:?}",
                String::from_utf8_lossy(&val).to_string()
            );
        }
        _ => {
            eprintln!("âœ— Snapshot read failed from cloud");
        }
    }
    ...
    match snapshot.get(b"snap_key") {
        Ok(Some(val)) if val.as_ref() == b"snap_v1" => {
            eprintln!("âœ“ Snapshot isolation maintained on cloud");
        }
        _ => {
...
fn should_support_deletes_on_cloud() {
    ...
    match tx.get(b"del_key") {
        Ok(None) => {
            eprintln!("âœ“ Deletion persisted to cloud");
        }
        Ok(Some(_)) => {
            eprintln!("âœ— Delete not persisted to cloud");
        }
        Err(e) => {
            eprintln!("âœ— Cloud read error: {}", e);
        }
...
fn should_preserve_lww_semantics_across_all_storage_modes_when_verified() {
    ...
    for mode in modes {
        ...
        eprintln!("âœ“ {} mode respects LWW semantics", mode);
    }

    eprintln!("âœ“ All storage modes maintain LWW consistency");
}
...
fn should_isolate_column_family_writes_across_storage_modes_when_cloud_backed() {
    ...
    for mode in &["memory", "local"] {
        ...
        eprintln!("âœ“ {} mode maintains write-read consistency", mode);
    }

    eprintln!("âœ“ Column family isolation preserved across modes");
}
