...
fn document_transaction_isolation_level_lww() {
    ...
    eprintln!("â•‘     MIDGE TRANSACTION ISOLATION LEVEL: LAST-WRITE-WINS        â•‘");
    ...
    eprintln!();
    eprintln!("âœ… What IS guaranteed:");
    ...
    eprintln!();
    eprintln!("âŒ What is NOT guaranteed:");
    ...
    eprintln!("  6. Last-Write-Wins  [âœ… IMPLEMENTED] <-- Midge");
    eprintln!();
    eprintln!("Use cases where LWW is appropriate:");
    eprintln!("  âœ… High-throughput distributed systems");
    eprintln!("  âœ… Time-series data (always using latest sensor reading)");
    eprintln!("  âœ… Cache-like usage patterns");
    eprintln!("  âœ… State that converges via last-write (e.g., configuration)");
    eprintln!();
    eprintln!("Use cases where LWW is DANGEROUS:");
    eprintln!("  âŒ Financial transactions (can lose money)");
    eprintln!("  âŒ Inventory management (can oversell)");
    eprintln!("  âŒ Voting/counting (can lose votes)");
    eprintln!("  âŒ Any aggregate that depends on all writes being counted");
    eprintln!();
}
