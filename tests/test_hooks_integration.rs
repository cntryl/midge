...
use cntryl_midge::{test_hooks::{TestHooks, FsyncBehavior}, MidgeEngine, MidgeOptions, StorageMode};
...
fn should_skip_fsync_when_test_hook_configured() {
    ...
    assert!(hooks.fsync_count() > 0, "fsync hooks should have been called");
...
fn should_record_fsync_calls_when_using_record_only_mode() {
    ...
    let dir = TempDir::new().unwrap();
    let hooks = TestHooks::new()
        .with_fsync_behavior(FsyncBehavior::RecordOnly);
...
fn should_increment_wal_append_count_when_writes_occur() {
    ...
    assert_eq!(final_count - initial_count, 3, "Expected 3 WAL appends for 3 puts");
}
...
fn should_call_manifest_hook_on_save() {
    ...
    manifest.save_atomic_with_hooks(dir.path(), Some(&hooks)).expect("save");
...
fn should_increment_compaction_counters_during_manual_compaction() {
    ...
    eng.wait_for_compaction(std::time::Duration::from_millis(100))
