//! Cloud Hybrid Storage Tests
//!
//! Tests for hybrid local/cloud storage behavior including cache eviction,
//! concurrent access, cache churn, and asynchronous upload handling.
//!
//! Test coverage:
//! - Cache eviction (LRU, size limits)
//! - Concurrent reads and writes to cache
//! - Cache churn correctness
//! - Asynchronous cloud uploads
//! - Cache recovery after deletion
...
fn should_evict_oldest_files_given_cache_full_when_adding_new_files() {
    ...
    // Assert - Cache should have evicted files
...
fn should_upload_to_cloud_asynchronously_without_blocking_writes() {
    ...
    // Wait for all uploads to reach the cloud
    use cntryl_midge::testkit::test_helpers::TEST_CLOUD_TIMEOUT;
