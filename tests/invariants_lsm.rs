...
#[test]
fn should_maintain_non_overlapping_sst_ranges_in_l1_plus() {
...
#[test]
fn should_sync_manifest_with_sst_directory() {
    for mode in disk_storage_modes() {
...
#[test]
fn should_prevent_orphan_sst_files() {
    for mode in disk_storage_modes() {
        let (_name, storage_mode, _temp_dir) = create_storage_mode(mode);
...
#[test]
fn should_recover_delete_range_correctly() {
    for mode in disk_storage_modes() {
        let (_name, storage_mode, _temp_dir) = create_storage_mode(mode);
