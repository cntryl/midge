...
fn run_workload_a(ctx: &mut StressContext, opts: MidgeOptions) {
    ...
    {
        let mut rng = ycsb::XorShift64::new(0xA0A0_AA11_5678_9ABC);
    ...
    // Expected: stable throughput; mem/local should be flat, cloud lower.
    let ops_done = Cell::new(0u64);
    let bytes_done = Cell::new(0u64);

    ctx.measure_ref(&engine, |e| {
        let mut rng = ycsb::XorShift64::new(0xA0A0_EA5E_5678_9ABC);
        let zipf = ZipfianGenerator::new(INITIAL_KEYS, ZIPFIAN_THETA);
...
#[stress_test]
fn tier4_ycsb_a_mem(ctx: &mut StressContext) {
    // Expected: highest throughput; stable latency.
    let opts = cntryl_midge::testkit::opts_for_mode("memory");
    run_workload_a(ctx, opts);
}

#[stress_test]
fn tier4_ycsb_a_local(ctx: &mut StressContext) {
    // Expected: slightly lower throughput than mem; occasional flush/compaction stalls.
    let opts = cntryl_midge::testkit::opts_for_mode("local");
    run_workload_a(ctx, opts);
}

#[stress_test]
fn tier4_ycsb_a_cloud(ctx: &mut StressContext) {
    // Expected: lowest throughput; higher tail latency from cloud durability.
