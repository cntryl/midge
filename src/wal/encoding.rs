...
pub fn encode(record: &WalRecord) -> MidgeResult<Bytes> {
    ...
    let mut flags = 0u8;
    if record.value.is_some() { flags |= 0x01; }
    if record.expiration.is_some() { flags |= 0x02; }
    if record.range_end.is_some() { flags |= 0x04; }
    if record.txn_id.is_some() { flags |= 0x08; }
    if record.compression.is_some() { flags |= 0x10; }
    buf.put_u8(flags);
