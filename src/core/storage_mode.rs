...
mod tests {
    ...
    #[test]
    fn should_identify_memory_mode() {
        // Arrange
        let mode = StorageMode::Memory;

        // Act
        let is_memory = mode.is_memory();
        let is_disk = mode.is_local_disk();
        let is_cloud = mode.is_cloud_backed();
    ...
    #[test]
    fn should_identify_local_disk_mode() {
        // Arrange
        let mode = StorageMode::LocalDisk {
            db_path: PathBuf::from("./target/tmp/test_db"),
        };
    ...
    #[test]
    fn should_identify_cloud_backed_mode() {
        use crate::cloud::MockCloudBackend;

        // Arrange
        let backend = Arc::new(MockCloudBackend::new());
        let mode = CloudStorageBuilder::new(PathBuf::from("./cache"), backend).build();
    ...
    #[test]
    fn should_return_local_path_for_memory_mode() {
        // Arrange
        let mem_mode = StorageMode::Memory;

        // Act
        let mem_path = mem_mode.local_path();

        // Assert
        assert!(mem_path.to_string_lossy().contains("midge-mem"));
    }

    #[test]
    fn should_return_local_path_for_local_disk_mode() {
        // Arrange
        let disk_mode = StorageMode::LocalDisk {
            db_path: PathBuf::from("./target/tmp/my_db"),
        };

        // Act
        let path = disk_mode.local_path();

        // Assert
        assert_eq!(path, PathBuf::from("./target/tmp/my_db"));
    }

    #[test]
    fn should_return_local_path_for_cloud_backed_mode() {
        use crate::cloud::MockCloudBackend;

        // Arrange
        let backend = Arc::new(MockCloudBackend::new());
        let cloud_mode = CloudStorageBuilder::new(PathBuf::from("./cloud_cache"), backend).build();

        // Act
        let path = cloud_mode.local_path();

        // Assert
        assert_eq!(path, PathBuf::from("./cloud_cache"));
    }

    #[test]
    fn should_build_cloud_mode_with_custom_settings() {
        use crate::cloud::MockCloudBackend;
        ...
        // Act
        let mode = CloudStorageBuilder::new(PathBuf::from("./cache"), backend)
            .with_path("customer-123".into())
            .with_local_wal_sync(false)
            .with_wal_batch_size(4_000_000)
            .with_sst_cache_capacity(200)
            .build();

        // Assert
        if let StorageMode::CloudBacked {
            storage_context,
            local_wal_sync,
            wal_batch_size,
            sst_cache_capacity,
        ...
        {
            assert_eq!(storage_context.path, "customer-123");
            assert!(!local_wal_sync);
            assert_eq!(wal_batch_size, 4_000_000);
            assert_eq!(sst_cache_capacity, 200);
        } else {
            panic!("Expected CloudBacked mode");
        }
    }
}
