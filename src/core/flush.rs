...
mod tests {
    ...
    // Helper to create EntryMeta
    fn make_entry(key: &[u8], seq: u64) -> crate::EntryMeta {
        crate::EntryMeta {
            key: key.to_vec(),
    ...
    #[test]
    fn should_return_none_given_empty_entries_when_computing_bounds() {
        // Arrange
        let entries: Vec<crate::EntryMeta> = vec![];
    ...
    #[test]
    fn should_compute_bounds_given_single_entry_when_computing_bounds() {
        // Arrange
        let entries = vec![make_entry(b"key1", 100)];
        let range_tombstones: Vec<(Vec<u8>, Vec<u8>, u64)> = vec![];
        ...
        assert_eq!(largest_key, Some(b"key1".to_vec()));
        assert_eq!(smallest_seq, Some(100));
        assert_eq!(largest_seq, Some(100));
    }

    #[test]
    fn should_compute_bounds_given_multiple_entries_when_computing_bounds() {
        // Arrange
        let entries = vec![
            make_entry(b"key3", 200),
            make_entry(b"key1", 100),
            make_entry(b"key2", 150),
        ];
        let range_tombstones: Vec<(Vec<u8>, Vec<u8>, u64)> = vec![];
        ...
        assert_eq!(smallest_seq, Some(100));
        assert_eq!(largest_seq, Some(200));
    }

    #[test]
    fn should_track_sequence_range_given_entries_when_computing_bounds() {
        // Arrange
        let entries = vec![
            make_entry(b"a", 500),
            make_entry(b"b", 100),
        ...
        // Act
        let (_sk, _lk, smallest_seq, largest_seq) = compute_bounds(&entries, &range_tombstones);

        // Assert
        assert_eq!(smallest_seq, Some(100));
        assert_eq!(largest_seq, Some(500));
    }

    #[test]
    fn should_include_range_tombstone_bounds_when_computing_bounds() {
        // Arrange
        let entries = vec![make_entry(b"key2", 100)];
        let range_tombstones = vec![(b"key1".to_vec(), b"key3".to_vec(), 200)];

        // Act
        let (smallest_key, largest_key, smallest_seq, largest_seq) =
            compute_bounds(&entries, &range_tombstones);

        // Assert: Range tombstone expands key bounds
        assert_eq!(smallest_key, Some(b"key1".to_vec())); // From range start
        assert_eq!(largest_key, Some(b"key3".to_vec())); // From range end
        assert_eq!(smallest_seq, Some(100));
        assert_eq!(largest_seq, Some(200));
    }

    #[test]
    fn should_compute_bounds_given_only_range_tombstones_when_computing_bounds() {
        // Arrange
        let entries: Vec<crate::EntryMeta> = vec![];
        let range_tombstones = vec![
            (b"a".to_vec(), b"m".to_vec(), 50),
            (b"n".to_vec(), b"z".to_vec(), 100),
        ];
        ...
        assert_eq!(smallest_seq, Some(50));
        assert_eq!(largest_seq, Some(100));
    }

    #[test]
    fn should_expand_bounds_given_range_tombstone_wider_than_entries() {
        // Arrange
        let entries = vec![make_entry(b"key5", 100), make_entry(b"key6", 150)];
        // Range tombstone covers keys before and after entries
        let range_tombstones = vec![(b"key1".to_vec(), b"key9".to_vec(), 200)];

        // Act
        let (smallest_key, largest_key, smallest_seq, largest_seq) =
            compute_bounds(&entries, &range_tombstones);

        // Assert: Range bounds expanded
        assert_eq!(smallest_key, Some(b"key1".to_vec()));
    ...
    let internal_key = crate::common::internal_key::encode_internal_key(b"user_key", 12345, false);
        ...
        // Act
        let (smallest_key, largest_key, _ss, _ls) = compute_bounds(&entries, &range_tombstones);

        // Assert: Decoded to user key
        assert_eq!(smallest_key, Some(b"user_key".to_vec()));
        assert_eq!(largest_key, Some(b"user_key".to_vec()));
    }

    #[test]
    fn should_combine_entries_and_range_tombstones_when_computing_bounds() {
        // Arrange
        let entries = vec![make_entry(b"d", 300), make_entry(b"e", 400)];
        let range_tombstones = vec![
            (b"a".to_vec(), b"c".to_vec(), 100),
            (b"f".to_vec(), b"z".to_vec(), 500),
        ];
