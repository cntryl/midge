...
Each layer (e.g., WAL, memtable, compaction, transaction manager) exposes its capabilities through interfaces like `KvStore` and `KvTransaction`, never through hidden globals or cross-module state.
...
- No logic should depend on "when" an operation runs, only on _what_ was requested.
...
- Compaction, snapshots, or expirations are _scheduled actions_, not background heuristics.
- Nothing happens "eventually" — every change originates from a clear command or deterministic policy.
- Background workers (flush, cloud upload, manifest write) are _event-driven_, not timer-based. They respond to explicit signals (e.g., memtable full, WAL rotation) rather than polling.
...
Wherever possible, subsystems are _pure state machines_:
...
> Midge favors _composition over coupling, determinism over timing, and explicit actions over side effects._
