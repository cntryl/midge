...
fn main() {
    ...
    println!(
        "Read Operations: {}",
        metrics.read_ops.load(std::sync::atomic::Ordering::Relaxed)
    );
    println!(
        "Write Operations: {}",
        metrics.write_ops.load(std::sync::atomic::Ordering::Relaxed)
    );
    println!(
        "Delete Operations: {}",
        metrics
    ...
    println!(
        "Write Latency (avg): {} ns",
        metrics.write_latency_ns.avg_nanos()
    );
    println!(
        "Read Latency (max): {} ns",
        metrics.read_latency_ns.max_nanos()
    );
    ...
    println!(
        "Compaction Duration (samples): {}",
        metrics.compaction_duration_ns.count()
    );
    ...
    println!(
        "WAL Syncs: {}",
        metrics.wal_syncs.load(std::sync::atomic::Ordering::Relaxed)
    );
    println!(
        "WAL Bytes Written: {}",
        metrics
