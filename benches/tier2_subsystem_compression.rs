...
#[path = "bench_support/adaptive_compression.rs"]
...
const SELECTED_FAST_ACCEPT_RATIO: f32 = 0.30;

macro_rules! adaptive_oracle_case {
    ($fn_name:ident, $size:expr, $shape:ident, $role:ident) => {
        #[stress(
            tier = 2,
            metadata(
                component = "compression",
                scenario = "adaptive_cascade_diagnostic",
                trust_class = "diagnostic",
                diagnostic_reason = "adaptive_cascade_failed_promotion"
            )
        )]
        fn $fn_name(ctx: &mut StressContext) {
            adaptive_compression::run_case(
                ctx,
                $size,
                adaptive_compression::CorpusShape::$shape,
                adaptive_compression::SelectionRole::$role,
                SELECTED_FAST_ACCEPT_RATIO,
            );
        }
    };
}

adaptive_oracle_case!(
    adaptive_cascade_candidate_repeated_16k,
    16 * 1024,
    Repeated,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_repeated_16k,
    16 * 1024,
    Repeated,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_structured_16k,
    16 * 1024,
    Structured,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_structured_16k,
    16 * 1024,
    Structured,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_mixed_16k,
    16 * 1024,
    Mixed,
    Candidate
);
adaptive_oracle_case!(adaptive_cascade_oracle_mixed_16k, 16 * 1024, Mixed, Oracle);
adaptive_oracle_case!(
    adaptive_cascade_candidate_prefix_random_tail_16k,
    16 * 1024,
    PrefixRandomTail,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_prefix_random_tail_16k,
    16 * 1024,
    PrefixRandomTail,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_uniform_16k,
    16 * 1024,
    Uniform,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_uniform_16k,
    16 * 1024,
    Uniform,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_repeated_64k,
    64 * 1024,
    Repeated,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_repeated_64k,
    64 * 1024,
    Repeated,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_structured_64k,
    64 * 1024,
    Structured,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_structured_64k,
    64 * 1024,
    Structured,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_mixed_64k,
    64 * 1024,
    Mixed,
    Candidate
);
adaptive_oracle_case!(adaptive_cascade_oracle_mixed_64k, 64 * 1024, Mixed, Oracle);
adaptive_oracle_case!(
    adaptive_cascade_candidate_prefix_random_tail_64k,
    64 * 1024,
    PrefixRandomTail,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_prefix_random_tail_64k,
    64 * 1024,
    PrefixRandomTail,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_uniform_64k,
    64 * 1024,
    Uniform,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_uniform_64k,
    64 * 1024,
    Uniform,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_low_cardinality_16k,
    16 * 1024,
    LowCardinality,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_low_cardinality_16k,
    16 * 1024,
    LowCardinality,
    Oracle
);
adaptive_oracle_case!(
    adaptive_cascade_candidate_low_cardinality_64k,
    64 * 1024,
    LowCardinality,
    Candidate
);
adaptive_oracle_case!(
    adaptive_cascade_oracle_low_cardinality_64k,
    64 * 1024,
    LowCardinality,
    Oracle
);

macro_rules! adaptive_threshold_screen {
    ($fn_name:ident, $ratio:expr) => {
        #[stress(
            tier = 2,
            metadata(
                component = "compression",
                scenario = "adaptive_threshold_screen",
                trust_class = "diagnostic",
                diagnostic_reason = "adaptive_cascade_threshold_screen"
            )
        )]
        fn $fn_name(ctx: &mut StressContext) {
            adaptive_compression::run_threshold_screen(ctx, $ratio);
        }
    };
}

adaptive_threshold_screen!(adaptive_threshold_screen_10, 0.10);
adaptive_threshold_screen!(adaptive_threshold_screen_20, 0.20);
adaptive_threshold_screen!(adaptive_threshold_screen_30, 0.30);
adaptive_threshold_screen!(adaptive_threshold_screen_40, 0.40);
adaptive_threshold_screen!(adaptive_threshold_screen_50, 0.50);
adaptive_threshold_screen!(adaptive_threshold_screen_60, 0.60);
adaptive_threshold_screen!(adaptive_threshold_screen_70, 0.70);
adaptive_threshold_screen!(adaptive_threshold_screen_80, 0.80);
