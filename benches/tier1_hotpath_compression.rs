...
fn run_decompress_raw(ctx: &mut StressContext, name: &'static str, algo: CompressionAlgo) {
    ...
    stress_config::measure_hot_path_batch(
        ctx,
        measurement_name,
        RAW_DECOMPRESS_BATCH_SIZE as u64,
        || {
            let mut bytes = 0usize;
            for _ in 0..RAW_DECOMPRESS_BATCH_SIZE {
...
fn run_compress_trailer(ctx: &mut StressContext, name: &'static str, policy: &CompressionPolicy) {
    ...
    stress_config::measure_hot_path_batch(
        ctx,
        measurement_name,
        TRAILER_COMPRESS_BATCH_SIZE as u64,
        || {
            let mut bytes = 0usize;
...
fn run_decompress_trailer(ctx: &mut StressContext, name: &'static str, policy: &CompressionPolicy) {
    ...
    stress_config::measure_hot_path_batch(
        ctx,
        measurement_name,
        TRAILER_DECOMPRESS_BATCH_SIZE as u64,
        || {
            let mut bytes = 0usize;
            for _ in 0..TRAILER_DECOMPRESS_BATCH_SIZE {
...
fn wal_compress_lz4_1kb(ctx: &mut StressContext) {
    ...
    stress_config::measure_hot_path_batch(
        ctx,
        "wal_compress_lz4_1kb",
        WAL_LZ4_BATCH_SIZE as u64,
        || {
            let mut bytes = 0usize;
            for _ in 0..WAL_LZ4_BATCH_SIZE {
                let (out, marker) = compress_wal_value(black_box(&medium_val));
                bytes = bytes.wrapping_add(out.len() + usize::from(marker.is_some()));
            }
            black_box(bytes);
        },
...
fn wal_decompress_passthrough_128b(ctx: &mut StressContext) {
    ...
    stress_config::measure_hot_path_batch(
        ctx,
        "wal_decompress_passthrough_128b",
        WAL_PASSTHROUGH_DECOMPRESS_BATCH_SIZE as u64,
        || {
            let mut bytes = 0usize;
            for _ in 0..WAL_PASSTHROUGH_DECOMPRESS_BATCH_SIZE {
...
fn wal_decompress_lz4_1kb(ctx: &mut StressContext) {
    ...
    stress_config::measure_hot_path_batch(
        ctx,
        "wal_decompress_lz4_1kb",
        WAL_LZ4_DECOMPRESS_BATCH_SIZE as u64,
        || {
            let mut bytes = 0usize;
            for _ in 0..WAL_LZ4_DECOMPRESS_BATCH_SIZE {
