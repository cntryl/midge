//! YCSB Workload B: 95% Read / 5% Write (Read-Heavy) - consolidated scenarios
...
fn bench_workload_b(c: &mut Criterion) {
    let mut group = c.benchmark_group("ycsb_workload_b");
    ...
    for &scenario in &scenarios {
        match scenario {
            "fs_nosync" => {
                ...
                group.bench_with_input(BenchmarkId::new("95r_5w", scenario), &engine, |b, e| {
                    b.iter(|| run_workload_b(e, OPS_PER_ITER, RECORD_COUNT))
                });
            }
            "fs_sync" => {
    ...
    for &threads in &THREAD_COUNTS {
        ...
        group.bench_with_input(
            BenchmarkId::new("concurrent_95r_5w", threads),
            &threads,
            |b, &_threads| {
                b.iter(|| {
